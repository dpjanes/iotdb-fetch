 ...
 *  Send XML
 */
const body_xml = _.promise.make(self => {
    const method = "body.xml";
...
const body_xml_p = xml => _.promise.make((self, done) => {
    const method = "body.xml.p";

    _.promise.make(self)
        .then(_.promise.add("document", xml))
        .then(body_xml)
        .then(_.promise.done(done))
        .catch(done)
})
