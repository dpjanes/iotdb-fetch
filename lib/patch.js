...
/**
 */
const patch = _.promise.make(self => {
    const method = "patch";

    assert.ok(self.url, `${method}: expected self.url`);

    self.__request = unirest.patch(self.url)
 ...
 *  Parameterized 
 */
const patch_p = (url, query) => _.promise.make((self, done) => {
    const method = "patch.p";
