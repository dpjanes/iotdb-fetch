 ...
 */
const document = method => paramd => _.promise.make((self, done) => {
    ...
    paramd.method = paramd.method || method;

    _.promise.make(self)
        .then(fetch[method].p(paramd.url))
        .then(_.promise.conditional(self.bearer_token, fetch.headers.authorization.bearer))
        .then(fetch.go)
        .then(_.promise.done(done, self, "url,headers,document,document_length,document_media_type,document_encoding"))
        .catch(done)
})
