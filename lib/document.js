...
const document = (http_method, is_json) => paramd => _.promise.make((self, done) => {
    ...
    [ "bearer_token", "accept", "headers", "form", "json" ].forEach(key => {
        if (paramd[key] !== true) {
            return;
        }
    ...
    _.promise.make(self)
        ...
        .then(_.promise.done(done, self, 
            is_json ? "url,json" : "url,headers,document,document_length,document_media_type,document_encoding,document_name"))
        .catch(done)
})
