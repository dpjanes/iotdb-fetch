...
const go = _.promise((self, done) => {
    ...
    const _doit = _url => {
        ...
        const options = {
            method: self.fetch$fetch.method,
            headers: Object.assign({}, self.fetch$fetch.headers),
        }

        if (self.fetch$fetch.query) {
            _.mapObject(self.fetch$fetch.query, (value, key) => {
        ...
        // const hash = _.hash.md5("iotdb-fetch.go", options.method, "@", url.toString(), "@", options.headers)
        ...
        const request = processor(url.toString(), options, response => {
            ...
            response.on('end', () => {
                ...
                if (!_.is.Empty(response.headers.location) && ((response.statusCode === 301) || (response.statusCode === 302))) {
                    if (self.fetch$fetch.options.nofollow) {
                        logger.debug({
                            method: go.method,
                            location: location,
                            url: _url,
                        }, "would redirect but 'nofollow' set")
                        // …fallthru…
                    } else if (++redirects > 4) {
        ...
        self.fetch$fetch.bodys.forEach(body => {
            request.write(body) 
        })
