...
/**
 */
const head = _.promise.make(self => {
    const method = "head";

    assert.ok(self.url, `${method}: expected self.url`);

    self.__request = unirest.head(self.url)
 ...
 */
const head_p = (url, query) => _.promise.make((self, done) => {
    const method = "head.p";
 ...
 */
const head_go = _.promise.make((self, done) => {
    _.promise.make(self)
        .then(head)
        .then(require("./go").go)
        .then(_.promise.done(done))
 ...
 */
exports.head = head;
exports.head.p = head_p;
exports.head.go = head_go;
