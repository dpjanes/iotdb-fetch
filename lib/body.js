...
const body_document = _.promise(self => {
    ...
    self.fetch$fetch.bodys = [ body ]
    self.fetch$fetch.headers["content-length"] = Buffer.byteLength(self.fetch$fetch.bodys[0])
    self.fetch$fetch.headers["content-type"] = `multipart/form-data; boundary=${boundary}`

    /*
    console.log("HERE:XXX.1", self.fetch$fetch)
...
const body_raw = _.promise(self => {
    ...
    self.fetch$fetch.bodys = [ self.document ]
    self.fetch$fetch.headers["content-length"] = Buffer.byteLength(self.fetch$fetch.bodys[0])
    self.fetch$fetch.headers["content-type"] = "application/octet-stream"
})
...
const body_json = _.promise(self => {
    ...
    self.fetch$fetch.bodys = [ JSON.stringify(self.json) ]
    self.fetch$fetch.headers["content-length"] = Buffer.byteLength(self.fetch$fetch.bodys[0])
    self.fetch$fetch.headers["content-type"] = "application/json"
})
...
const body_xml = _.promise(self => {
    ...
    self.fetch$fetch.bodys = [ self.xml ]
    self.fetch$fetch.headers["content-length"] = Buffer.byteLength(self.fetch$fetch.bodys[0])
    self.fetch$fetch.headers["content-type"] = "text/xml"
})
...
const body_form = _.promise(self => {
    ...
    self.fetch$fetch.bodys = [ querystring.stringify(self.json) ]
    self.fetch$fetch.headers["content-length"] = Buffer.byteLength(self.fetch$fetch.bodys[0])
    self.fetch$fetch.headers["content-type"] = "application/x-www-form-urlencoded"
})
